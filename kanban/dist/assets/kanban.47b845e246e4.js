const state = {
    ...
    const DATA_STORAGE_KEY = 'mao-kanban-data-v1';
    ...
    function renderConfig() {
      ...
      const modeText = state.serviceMode ? '服务模式已连接' : '静态模式';
      const authText = config.authReady ? '飞书凭证已配置' : (state.serviceMode ? '请配置飞书 App ID / Secret' : '配置已保存在当前浏览器');
      const reviewText = config.reviewUrl ? '复盘表已配置' : '复盘表未配置';
      document.getElementById('configStatus').textContent = modeText + ' · ' + authText + ' · ' + reviewText;
    }
    ...
    function normalizeConfig(config) {
      return {
        ...
        appSecret: config?.appSecret ? String(config.appSecret).trim() : undefined,
        ...
        serviceMode: Boolean(config?.serviceMode),
      };
    }
    ...
    async function loadConfig() {
      ...
      try {
        ...
        if (!response.ok || serverConfig.error) throw new Error(serverConfig.error || '服务配置读取失败');
        state.serviceMode = true;
        state.config = normalizeConfig({ ...state.config, ...serverConfig, dataUrl: serverConfig.dataUrl || '/api/kanban-data' });
    ...
    function normalizeDataPayload(payload) {
      if (!payload || typeof payload !== 'object') throw new Error('看板 JSON 不是有效对象');
      const days = payload.days && typeof payload.days === 'object' ? payload.days : {};
      const dates = Array.isArray(payload.dates) && payload.dates.length
        ? payload.dates
    ...
    function readStoredData() {
      ...
      const payload = normalizeDataPayload(stored?.payload || stored);
      return {
        payload,
        savedAt: stored?.savedAt || payload.refreshedAt || '',
      };
    }

    function storedDataLabel(stored) {
      if (!stored?.savedAt) return '本地缓存';
      return '本地缓存 ' + new Date(stored.savedAt).toLocaleString('zh-CN', { hour12: false });
    }

    function applyDataPayload(payload, sourceLabel = '') {
      state.data = normalizeDataPayload(payload);
      state.dataSourceLabel = sourceLabel;
      window.KANBAN_DAILY_DATA = state.data.days || {};
      window.KANBAN_SOURCE = state.data.source || {};
      if (!state.date || !state.data.dates.includes(state.date)) {
        state.date = state.data.dates[state.data.dates.length - 1] || '';
      }
      if (!state.date) showMessage('没有可展示的日期数据。');
      render();
    }

    function saveDataToLocalStorage(payload, { renderNow = true } = {}) {
      const normalized = normalizeDataPayload(payload);
      const savedAt = new Date().toISOString();
      localStorage.setItem(DATA_STORAGE_KEY, JSON.stringify({ savedAt, payload: normalized }));
      if (renderNow) applyDataPayload(normalized, '本地缓存 ' + new Date(savedAt).toLocaleString('zh-CN', { hour12: false }));
      return normalized;
    }
    ...
    async function saveConfig() {
      ...
      try {
        const formConfig = readConfigForm();
        if (state.serviceMode) {
          ...
          if (!response.ok || payload.error) throw new Error(payload.error || '服务配置保存失败');
        ...
        localStorage.setItem(CONFIG_STORAGE_KEY, JSON.stringify(publicConfig(state.config), null, 2));
        renderConfig();
        document.getElementById('configStatus').textContent = state.serviceMode ? '已保存到服务端' : '已保存到当前浏览器';
        await loadData(true);
      } catch (error) {
    ...
    async function loadData(forceRefresh = false) {
      ...
      try {
        ...
        try {
          ...
          console.warn('Ignored invalid kanban local data:', error);
        }
        if (!forceRefresh && stored?.payload) {
          applyDataPayload(stored.payload, storedDataLabel(stored));
          showMessage('');
          return;
        }
        ...
        const normalized = saveDataToLocalStorage(payload, { renderNow: false });
        applyDataPayload(
          normalized,
          forceRefresh
            ? '数据 JSON 已刷新 ' + new Date().toLocaleString('zh-CN', { hour12: false })
            : '静态 JSON 兜底'
        );
        showMessage('');
      } catch (error) {
        if (forceRefresh) {
          try {
            ...
            if (stored?.payload) {
              applyDataPayload(stored.payload, storedDataLabel(stored));
              showMessage('刷新数据 JSON 失败，已显示本地缓存：' + esc(error.message), 'error');
              return;
            }
          } catch {}
        }
    ...
    function renderSourceStatus() {
      ...
      const sourceLabel = state.dataSourceLabel ? ' · ' + state.dataSourceLabel : '';
      document.getElementById('sourceStatus').textContent = source
        ? 'raw ' + source.rawRowCount + ' 行 · 手动 ' + (source.manualInputRowCount || 0) + ' 行 · 大看板 ' + source.bigBoardFieldCount + ' 字段 · 单品 ' + source.smallBoardFieldCount + ' 字段 · ' + refreshed + sourceLabel
        : (state.dataSourceLabel || '读取静态看板数据');
    }
    ...
    window.MaoKanban = {
      dataStorageKey: DATA_STORAGE_KEY,
      configStorageKey: CONFIG_STORAGE_KEY,
      setData: (payload) => saveDataToLocalStorage(payload),
      getData: () => readStoredData()?.payload || null,
      clearData: async () => {
        localStorage.removeItem(DATA_STORAGE_KEY);
        state.data = null;
